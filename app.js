...
function parseTrainNo(trainNo) {
  ...
  for (const row of state.datasets.trainTable) {
    ...
    if (!isNaN(start) && !isNaN(end) && n >= start && n <= end) {
      const type = row["種別"] || "";
...
function screenSettings() {
  ...
  const trainNo = el("input",{type:"text",placeholder:"4桁の列車番号",id:"trainNo"});
  ...
  state.datasets.types.forEach(t=>typeSel.appendChild(el("option",{value:t},t)));

  const destSel = el("select",{id:"dest"});
  state.datasets.dests.forEach(d=>destSel.appendChild(el("option",{value:d},d)));

  const carsIn = el("input",{type:"number",id:"cars",min:"1",max:"12",value:"10"});

  const endChange = el("input",{type:"checkbox",id:"endChange"});
  const secondWrap = el("div",{id:"secondConfig",style:"display:none;"});
  endChange.onchange =()=> secondWrap.style.display = endChange.checked ? "block":"none";

  const trainNo2 = el("input",{type:"text"});
  const typeSel2 = el("select");
  state.datasets.types.forEach(t=>typeSel2.appendChild(el("option",{value:t},t)));
  ...
  c.append(
    el("div",{class:"row"},[el("label",{},"列車番号"),trainNo,btnSearch]),
    el("div",{class:"grid2"},[
...
function screenStart() {
  ...
  root.append(el("div",{class:"centered"},[
...
function band1RenderCars(elm, show, cars){
  elm.innerHTML="";
  if(!show) return;
  const c = cars==10?"d10":cars==8?"d8":cars==6?"d6":cars==4?"d4":cars==7?"d7":"d10";
  const diamond = el("div",{class:`diamond ${c}`},[el("span",{},cars==7?"特":`${cars}両`)]);
...
function screenGuidance(){
  ...
  const band2 = el("div",{class:"band band2"},[el("div",{class:"notes",id:"notes"},"")]);
  const band3 = el("div",{class:"band band3"},[el("div",{class:"badge",id:"badgeType"},"")]);
  ...
  const modal = el("div",{class:"modal",id:"menuModal"},[
    el("div",{class:"panel"},[
      ...
      el("div",{class:"list"},[
        ...
        el("button",{class:"btn secondary",id:"m-platform"},"着発線変更"),
  ...
  modal.onclick = e=>{ if(e.target.id==="menuModal") modal.classList.remove("active"); };
  ...
  modal.querySelector("#m-dest").onclick =()=> openList("行先変更",state.datasets.dests,v=>state.config.dest=v);
  modal.querySelector("#m-type").onclick =()=> openList("種別変更",state.datasets.types,v=>state.config.type=v);
...
function openList(title, list, onPick){
  ...
  const wrap = el("div",{},[
    el("hr",{class:"sep"}),
    el("h3",{},title),
    el("div",{class:"list"}, list.map(v=>{
      const b=el("button",{class:"btn secondary"},v);
      b.onclick =()=>{ onPick(v); modal.classList.remove("active"); };
      return b;
    }))
  ]);
  panel.appendChild(wrap);
  modal.addEventListener("transitionend",()=>panel.removeChild(wrap),{once:true});
}
...
function openStopList(){
  ...
  const done = el("button",{class:"btn"},"決定");
    ...
    checks.forEach((c,i)=>{ if(c.checked) newSel.add(names[i]); });
...
let clockTimer=null, gpsWatchId=null;
...
function nearestStation(lat,lng){
  ...
  for(const [name,info] of Object.entries(state.datasets.stations)){
    ...
    if(d<bestD){bestD=d;best={name,...info,distance:d};}
  }
  return best;
}
...
function onPos(pos){
  ...
  if(state.runtime.lastPosition){
    ...
    const dist=haversine(state.runtime.lastPosition.lat,state.runtime.lastPosition.lng,latitude,longitude);
  ...
  maybeSpeak(ns);
  const show = ns && ns.distance<=300 && !state.runtime.passStations.has(ns.name);
  band1RenderCars(document.getElementById("screen-guidance")._band1,show,state.config.cars);
}
...
function maybeSpeak(ns){
  ...
  if(!isNonPassenger(t)){
    ...
    if(state.runtime.lastStopStation && ns.distance>100){
      speakOnce("leave100_"+key,`次は${state.runtime.lastStopStation}、停車`);
      state.runtime.lastStopStation=null;
    }

    if(isStop && ns.distance<=300){
      speakOnce("arr300_"+key,`まもなく${ns.name}、停車、${state.config.cars}両`);
    }

    if(isStop && ns.distance<=120){
      if(state.config.cars===8 && (state.config.direction==="上り"?ns.up8pos:ns.down8pos)){
        speakOnce("arr120_"+key,`停車、8両、${(state.config.direction==="上り")?ns.up8pos:ns.down8pos}あわせ`);
      } else if(state.config.cars===10){
        speakOnce("arr120_"+key,`停車、10両`);
      } else {
        speakOnce("arr120_"+key,`停車、${state.config.cars}両、停止位置注意`);
      }
    ...
    if(ns.distance<=200 && d<=45) speakOnce("nonp200_"+key,`種別回送、ていつう確認`);
    if(ns.distance<=120 && d<=30) speakOnce("nonp120_"+key,`種別回送、ドアあつかい注意`);
  }
}
...
function otherSpeaks(ns){
  ...
  if(state.config.direction==="上り" && state.config.dest==="小竹向原" && ns.name==="練馬" && ns.distance>100){
    speakOnce("rule-nerima","搭載かばん、確認");
  }
  if(state.config.direction==="上り" && /新宿線/.test(state.config.dest) && ns.name==="所沢" && ns.distance<=400){
    speakOnce("rule-tokorozawa","列車無線チャンネル切り替え");
  }
  if(state.config.direction==="上り" && state.config.dest==="池袋" && timeOK && ns.name==="椎名町" && ns.distance>150){
